...
def is_token_limit(stop_reason: Optional[str]) -> bool:
    if not stop_reason:
        return False
    s = stop_reason.lower()
    return any(t in s for t in ["max_tokens", "length", "limit", "token"])
...
def call_llm_until_complete_with_chunks(base_url: str, token: str, model: str, user_prompt: str, max_tokens: int):
    accumulated = ""
    ...
    while True:
        ...
        if stop_reason and is_token_limit(stop_reason):
            current_prompt = (
                ...
                + accumulated
                + "\n\nContinue the output from where it stopped and do not add any extra comments."
            )
            print(f"Chunk {chunks} completed")
            continue
...
def convert_sql_file(base_url: str, token: str, model: str, sql_path: str, user_prompt: str, max_tokens: int = 5000) -> str:
    ...
    final_output , chunks = call_llm_until_complete_with_chunks(
        base_url=base_url,
        token=token,
        model=model,
    ...
    print(f"Completed in {chunks} chunks")
    print(f"Time taken: {int(minutes)} min {seconds:.2f} sec")
...
print(f"Stored Procedure Migration Completed")
